...
2. **Fill in the templates.** Work top-down:
   - `core/` — who you are. Optional; leave it blank and everything else still works.
   - `stack/` — your machine, shell, tools, hardware. Replace the example files with yours; delete what doesn't apply.
   - `taste/` — your writing voice, code style, naming. Swap the demo corpus for your own samples (see below).
   - `workflows/` — adjust the project and work-management conventions to how you actually work.
...
1. Add one pointer to that root file:
   > Read `lib-me/AGENTS.md` first, then `lib-me/workflows/agents-init.md`.
2. Let the agent follow [`workflows/agents-init.md`](workflows/agents-init.md). It specifies how to:
...
`taste/corpus/` holds short writing samples used as few-shot examples so agents can match your voice. The samples shipped in this public repo are **neutral demonstrations** — they show the mechanism, not anyone's real writing.
